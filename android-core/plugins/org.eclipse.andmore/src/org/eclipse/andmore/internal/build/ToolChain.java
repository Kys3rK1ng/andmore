...
import org.eclipse.andmore.AndmoreHome;
...
public class ToolChain {
	 ...
	 */
    private class ErrorLogger implements ILogger {

		@Override
		public void error(Throwable throwable, String format, Object... args) {
			AndmoreAndroidPlugin.logAndPrintError(throwable, javaProject.getElementName(), format, args);
		}
    ...
    /** The plug-in ID used for error reporting */
	private static final String PLUGIN_ID = "org.eclipse.andmore"; 
    ...
    public ToolChain(IJavaProject javaProject, 
        ...
        try {
        	...
        	File andmoreHomePath = new AndmoreHome(logger).getAndmoreHomePath();
	...
	public void executeDx(Collection<String> inputPaths,
    	...
    	// Flag whether Java version requires desuar to be included in the tool chain
...
*/            
    ...
    /**
     * Retruns Workbench factory instance
     * @return
     */
   ...
   /**
            ...
            Collection<String> osFilenames) throws CoreException, IOException
    {
        ...
        return runJavaMain(DEX_CONFIG_NAME, DX_CLASS, "-Djava.awt.headless=true -Xmx1024M -Dfile.encoding=UTF-8", dxJarPath, command);
    }
    ...
    protected int runJar(String configName, File jar, String main, String vmArgs,  Collection<String> arguments) throws CoreException {
        ...
        catch (DebugException e)
        {
            ...
            throw new CoreException(new Status(IStatus.ERROR, PLUGIN_ID, message, e));
        }
        return returnCode;
    }
	...
	private void appendAndroidBootFiles(Collection<String> commandSequence) {
		...
		File androidJar = projectState.getTarget().getFile(IAndroidTarget.ANDROID_JAR);
	...
	private void appendJreBootFiles(Collection<String> commandSequence) {
		String[] jreBootFiles = System.getProperty("sun.boot.class.path").split(File.pathSeparator);
